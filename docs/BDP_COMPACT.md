...
./scripts/build-bdp-compact.sh

# 출력 파일:
# - dist/bdp_compact/bdp_compact-1.0.0-py3-none-any.whl
# - dist/bdp_compact/lambda-layer.zip
```
...
| **Memory** | 512MB |
| **Timeout** | 120s |
| **Handler** | `bdp_compact.handler.handler` |
| **Layer** | `dist/bdp_compact/lambda-layer.zip` |

### FastAPI 서버 실행

```bash
# 개발 모드
python -m src.agents.bdp_compact.server

# 또는
uvicorn src.agents.bdp_compact.server:app --port 8005 --reload

# Production
uvicorn src.agents.bdp_compact.server:app --host 0.0.0.0 --port 8005 --workers 4
```
...
WORKDIR /app
COPY dist/bdp_compact/bdp_compact-1.0.0-py3-none-any.whl .
RUN pip install bdp_compact-1.0.0-py3-none-any.whl

EXPOSE 8005
CMD ["uvicorn", "bdp_compact.server:app", "--host", "0.0.0.0", "--port", "8005"]
```
...
```bash
# 전체 테스트
pytest tests/agents/bdp_compact/ -v

# 탐지 알고리즘 테스트
pytest tests/agents/bdp_compact/test_anomaly_detector.py -v

# Summary 생성 테스트
pytest tests/agents/bdp_compact/test_summary_generator.py -v
```
...
# Handler 테스트
python -c "
from src.agents.bdp_compact.handler import handler
...
# 통합 테스트
pytest tests/agents/bdp_compact/test_integration.py -v
```
