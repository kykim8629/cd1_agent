...
# 패턴 인식기 테스트
pytest tests/agents/bdp_compact/test_pattern_recognizers.py -v

# Summary 생성 테스트
pytest tests/agents/bdp_compact/test_summary_generator.py -v

# Handler 테스트
pytest tests/agents/bdp_compact/test_handler.py -v
```
...
pytest tests/agents/bdp_compact/test_localstack_integration.py -v
```
...
```bash
# 커버리지 측정
pytest tests/agents/bdp_compact/ --cov=src/agents/bdp_compact --cov-report=html
...
on:
  push:
    paths:
      - 'src/agents/bdp_compact/**'
      - 'tests/agents/bdp_compact/**'
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run tests
        run: |
          pytest tests/agents/bdp_compact/ -v --cov=src/agents/bdp_compact
