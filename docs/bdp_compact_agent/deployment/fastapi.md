...
RUN pip install --no-cache-dir bdp_compact-1.0.0-py3-none-any.whl
...
```bash
# 빌드
docker build -t bdp-compact-agent:latest .
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: bdp-compact
          image: bdp-compact-agent:latest
          ports:
            - containerPort: 8005
          env:
            - name: BDP_PROVIDER
