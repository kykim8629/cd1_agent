"""
BDP Detection DAG - AWS CloudWatch Log/Metric Anomaly Detection.

This DAG triggers the BDP Agent Lambda function every 5 minutes
to detect anomalies in AWS CloudWatch logs and metrics.
"""
...
def check_anomalies_found(**context):
    """Check if anomalies were found in detection result."""
    ti = context["ti"]
    detection_result = ti.xcom_pull(task_ids="detect_bdp_anomalies")
...
with DAG(
    dag_id="bdp_detection_dag",
    default_args=default_args,
    description="BDP Agent - AWS CloudWatch Log/Metric Anomaly Detection",
    ...
    # Task 1: Detect anomalies
    detect_anomalies = LambdaInvokeFunctionOperator(
        task_id="detect_bdp_anomalies",
    ...
    check_result = BranchPythonOperator(
        ...
        python_callable=check_anomalies_found,
        provide_context=True,
    )

    # Task 3a: Start Step Functions workflow if anomalies found
    start_workflow = StepFunctionStartExecutionOperator(
        task_id="start_workflow",
        state_machine_arn="{{ var.value.bdp_workflow_arn }}",
        input=json.dumps({
            "source": "mwaa",
            "dag_id": "bdp_detection_dag",
            "execution_date": "{{ ds }}",
